...
def plot_rho(Ts, Hs, rho_xxs, cutoff, resu_dir):
    ...
    axs.set_xlabel(r'$H$ [T]', fontsize = 14)
    axs.set_ylabel(r'$\rho_{xx}$ [$\Omega$m]', fontsize = 14)
    axs.tick_params(which = 'both', direction = 'in', top = False, right = False, length = 5, width = 1.5, labelsize = 14)
    axs.set_xlim((np.min(Hs[cutoff:]), np.max(Hs[cutoff:])))
    axs.yaxis.get_offset_text().set_size(14)

    cb = f.colorbar(plt.cm.ScalarMappable(cmap = cmap, norm = plt.Normalize(vmin = 0, vmax = np.max(Ts))))
    cb.ax.set_title(r'$T$ (K)')
    cb.ax.tick_params(length = 5, width = 1.5, labelsize = 14)
...
def plot_MR(Ts, Hs, MRs, cutoff, resu_dir):
    ...
    axs.set_xlabel(r'$H$ [T]', fontsize = 14)
    axs.set_ylabel(r'$MR$ [%]', fontsize = 14)
    axs.tick_params(which = 'both', direction = 'in', top = False, right = False, length = 5, width = 1.5, labelsize = 14)
...
def plot_MRK(Ts, Hs, MRs, rho_xx0s, cutoff, resu_dir):
    ...
    axs.set_xlabel(r'$H/\rho_{0}$ [T$\Omega^{-1}$m$^{-1}$]', fontsize = 14)
    axs.set_ylabel(r'$MR$ [%]', fontsize = 14)
    axs.tick_params(which = 'both', direction = 'in', top = False, right = False, length = 5, width = 1.5, labelsize = 14)
    axs.ticklabel_format(axis = 'y', style = 'sci', scilimits = (0, 0), useMathText=True)
    axs.yaxis.get_offset_text().set_size(14)
...
def plot_MREK(Ts, Hs, MRs, rho_xx0s, cutoff, resu_dir):
    ...
    axs.set_xlabel(r'$H/\rho_{0}n_T$ [$T\cdot\Omega^{-1}\cdot m^{-1}$]', fontsize = 14)
    axs.set_ylabel(r'$MR$ [%]', fontsize = 14)
    axs.tick_params(which = 'both', direction = 'in', top = False, right = False, length = 5, width = 1.5, labelsize = 14)
    axs.ticklabel_format(axis = 'y', style = 'sci', scilimits = (0, 0), useMathText=True)
    axs.yaxis.get_offset_text().set_size(14)
    ...
    axs.set_xlabel(r'$T$ [K]', fontsize = 14)
    axs.set_ylabel(r'$n_T$ []', fontsize = 14)
    axs.tick_params(which = 'both', direction = 'in', top = False, right = False, length = 5, width = 1.5, labelsize = 14)
    axs.yaxis.get_offset_text().set_size(14)
